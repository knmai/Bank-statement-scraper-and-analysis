...
def create_df_convert_csv(input):
    ...
    try:
        df_list = tabula.read_pdf(input, stream = True, guess = True, pages = 'all',
    ...
    #Adding missing column as row to data frame
    df_fin.loc[len(df_fin.index)] = ['11/16/2021', 'Deposit', 'Deposit', '$3,200.49', '$3,200.49'] 

    return df_fin.to_csv('november_22_transactions.csv')

create_df_convert_csv('november21.pdf')
